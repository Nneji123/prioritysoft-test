...
def employee_user(db):
    return User.objects.create_user(
        email="user@example.com", password="password123", is_employee=True
    )
...
def item(db):
    return Item.objects.create(
        name="Test Item", description="This is a test item.", price=9.99
    )
...
def test_create_item(api_client, employee_user):
    api_client.force_authenticate(user=employee_user)
    data = {"name": "New Item", "description": "A new item", "price": 19.99}
